...
par(mfcol = c(7,3))
for (val in categorical_variables) {
  ggplot(data=data)+aes_string(val)+geom_bar(aes(fill=as.factor(AttritionF)), position="fill")+ ggtitle(glue("{val}"))
}
...
for (val in categorical_variables) {
  ...
  p[[i]] <- ggplot(data=data)+aes_string(val)+geom_bar(aes(fill=as.factor(AttritionF)), position="fill")+ ggtitle(glue("{val}"))
  i = i + 1
  print(i)
  #print(plot)
}
...
for (val in categorical_variables) {
  ...
  plots[[i]] <- ggplot(data=data)+aes_string(val)+geom_bar(aes(fill=as.factor(AttritionF)), position="fill")+ ggtitle(glue("{val}"))
  i = i + 1
  #print(plot)
}
library(multiplot)
