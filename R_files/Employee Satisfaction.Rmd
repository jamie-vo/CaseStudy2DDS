...
```{r}
variables_graphed <- subset(data, select = c(ID, Attrition, JobRole, MaritalStatus, OverTime, JobLevel, JobInvolvement, StockOptionLevel, WorkLifeBalance, MonthlyIncome, PercentSalaryHike, TotalWorkingYears, YearsAtCompany, YearsSinceLastPromotion))
...
#split data
trainIndices = sample(1:dim(prepped_df)[1],round(splitPerc * dim(prepped_df)[1]))
train = prepped_df[trainIndices,]
test = prepped_df[-trainIndices,]
...
for(i in 1:15)
{
  classifications = knn(train[,6:70],test[,6:70],train$Attrition, prob = TRUE, k = i)
...
for(i in 1:20)
{
  ...
  classifications = knn(train[,6:70],test[,6:70],train$Attrition, prob = TRUE, k = 15)
  table(test$Attrition,classifications)
  CM = confusionMatrix(table(test$Attrition,classifications))
  accs$accuracy[i] = CM$overall[1]
  accs$specificity[i] = CM$byClass[2]
...
prepped_df <- subset(variables_graphed, select = -c(JobRole, MaritalStatus, OverTime, JobLevel, JobInvolvement, StockOptionLevel, WorkLifeBalance))
need_dummycode <- c("JobRole", "MaritalStatus", "OverTime", "JobLevel", "JobInvolvement", "StockOptionLevel", "WorkLifeBalance")
for (val in need_dummcode) {
  temp <- as.data.frame(dummy.code(data[["JobRole"]]))
  prepped_df <- cbind(prepped_df, temp)
}
head(prepped_df)
```
